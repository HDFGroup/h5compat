...
command needed is:
	...
	The tests will, by default, use the following HDF5 libraries on kagiso
that are compiled for daily tests.
...
one or both of the variables "h5cc18" and "h5cc16", as above.  The variable
...
different effect that the new format might have on version compatibility.
...
re-generated with v1.6 before each test is run.
 ...
 READING / CHECKING THE FILE
	...
	To determine whether the modified file is still readable and
compatibile with the v1.6 library, the file is read using read_compat.c
(compiled with v1.6).  

	read_compat.c generates a file called errors.log, which contains the
results of every attempt to read part of the file.  Each line in errors.log
contains 4 parts.

1) either Error or Passed
2) the type of object trying to be read (file, group, dataset, etc.)
3) the name of the object being read (g1, dset1, hlink1, etc.)
4) an optional comment about the specific read attempt.
